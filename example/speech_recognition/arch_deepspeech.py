...
def arch(args, seq_len=None):
    ...
    if isinstance(args, argparse.Namespace):
        ...
        if mode == "train" or is_bucketing:
            ...
            net = conv(net=net, channels=channel_num, filter_dimension=conv_layer1_filter_dim,
                       stride=conv_layer1_stride, no_bias=is_batchnorm, name='conv1')
            if is_batchnorm:
                # batch norm normalizes axis 1
                net = batchnorm(net, name="conv1_batchnorm")

            net = conv(net=net, channels=channel_num, filter_dimension=conv_layer2_filter_dim,
                       stride=conv_layer2_stride, no_bias=is_batchnorm, name='conv2')
            if is_batchnorm:
                # batch norm normalizes axis 1
            ...
            if rnn_type == "bilstm":
                net = bi_lstm_unroll(net=net, seq_len=seq_len_after_conv_layer2,
                                     num_lstm_layer=num_rnn_layer, dropout=0., is_batchnorm=is_batchnorm,
                                     is_bucketing=is_bucketing, num_hidden_lstm_list=num_hidden_rnn_list)
            elif rnn_type == "gru":
                net = gru_unroll(net=net, seq_len=seq_len_after_conv_layer2, num_hidden_gru_list=num_hidden_rnn_list,
                                 num_gru_layer=num_rnn_layer, dropout=0., is_batchnorm=is_batchnorm,
                                 is_bucketing=is_bucketing)
            elif rnn_type == "bigru":
                net = bi_gru_unroll(net=net, seq_len=seq_len_after_conv_layer2, num_hidden_gru_list=num_hidden_rnn_list,
                                    num_gru_layer=num_rnn_layer, dropout=0., is_batchnorm=is_batchnorm,
                                    is_bucketing=is_bucketing)
            else:
            ...
            # warpctc layer
            net = warpctc_layer(net=net, seq_len=seq_len_after_conv_layer2, label=label, num_label=num_label,
