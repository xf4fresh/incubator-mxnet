...
class VAE:
    ...
    def Bernoulli(x_hat, loss_label):
        return (-mx.symbol.sum(
            mx.symbol.broadcast_mul(loss_label, mx.symbol.log(x_hat)) + mx.symbol.broadcast_mul(1 - loss_label,
                                                                                                mx.symbol.log(
                                                                                                    1 - x_hat)),
            axis=1))
    ...
    def __init__(self, n_latent=5, num_hidden_ecoder=400, num_hidden_decoder=400, x_train=None, x_valid=None,
        ...
        self.model, self.training_loss = VAE.train_vae(x_train, x_valid, batch_size, n_latent, num_hidden_ecoder,
                                                       num_hidden_decoder, learning_rate, weight_decay, num_epoch,
                                                       optimizer, model_prefix, likelihood, initializer)
    ...
    def train_vae(x_train, x_valid, batch_size, n_latent, num_hidden_ecoder, num_hidden_decoder, learning_rate,
        ...
        if x_valid is not None:
            nd_iter_val = mx.io.NDArrayIter(data={'data': x_valid}, label={'loss_label': x_valid},
        ...
        model.fit(nd_iter,  # train data
                  ...
                  batch_end_callback=log_to_list(int(N / batch_size), training_loss),
                  # this can save the training loss
                  num_epoch=num_epoch,
                  eval_metric='Loss')
    ...
    def encoder(model, x):
        ...
        encoder_h = np.dot(params['encoder_h_weight'].asnumpy(), np.transpose(x)) + np.reshape(
            params['encoder_h_bias'].asnumpy(), (encoder_n, 1))
    ...
    def decoder(model, z):
        ...
        decoder_z = np.dot(params['decoder_z_weight'].asnumpy(), np.transpose(z)) + np.reshape(
            params['decoder_z_bias'].asnumpy(), (decoder_n, 1))
        act_z = np.tanh(decoder_z)
        decoder_x = np.transpose(np.dot(params['decoder_x_weight'].asnumpy(), act_z)) + params[
            'decoder_x_bias'].asnumpy()
